...
export const MOCK_PRODUCT_CODES: Record<string, string[]> = {
  ...
  Cold: [
    "10CR3", "10CR1HC", "20CR1HC", "20CR3", "30CR1HC", "40CR3HC", 
...
export function getDynamicProductCodes(unitType: string, availableCodes: Set<string>): {
  ...
  const sortedWithData = Array.from(new Set(hardcodedForType)).sort();

  return {
    withData: sortedWithData,
    withoutData: [],
    validCodesForType: sortedWithData,
  };
}
